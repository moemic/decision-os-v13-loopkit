...
## 通常の選択で使われた後日観測 — 2026-09-19

[一件の許可済みRun](recycle_aspire_scan_unavailability.md)では、旧Run 3の配置不一致を候補として
検討し、この後日結果と現在の修正blob同一性から重複修復を除いた。その後、別に実観測した
scanの未取得理由表示を修復した。索引の観測003に、元の実行IDを変えず接続している。
これは実際の候補除外と別作業の実行まで使われた観測であり、旧001の効果未確認や002の
read-only試用を上書きしない。記憶なしとの比較、削減時間、一般的な選択成功は未確認。
FN125の版・成功数、外部O-69 transportの状態、当時の合理性判断は変更しない。
